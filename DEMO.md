...
```bash
cd backend-spring && ./mvnw test    # 37 testes verdes
```
São 127 testes automatizados no projeto: 37 no backend, 17 no painel, 6 de ponta a ponta no navegador, 7 no app React Native e 60 no app Flutter.
