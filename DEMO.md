...
```bash
cd backend-spring && ./mvnw test    # 50 testes verdes
```
São 140 testes automatizados no projeto: 50 no backend, 17 no painel, 6 de ponta a ponta no navegador, 7 no app React Native e 60 no app Flutter.
