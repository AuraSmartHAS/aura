...
```bash
cd backend-spring && ./mvnw test    # 11 testes verdes
```
São 54 testes automatizados no projeto: 11 no backend, 17 no painel, 6 de ponta a ponta no navegador, 7 no app React Native e 13 no app Flutter.
